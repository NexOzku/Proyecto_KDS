// API Configuration (usando constantes de index.js)
...
const nroDocument = document.getElementById('nroDocument');
...
documentTypeId.addEventListener('blur', function() {
  const isValid = this.value !== '';
  this.parentElement.classList.toggle('error-active', !isValid);
});

// ✅ Validar Número de Documento
nroDocument.addEventListener('blur', function() {
...
function checkFormValidity() {
  ...
  const docType = documentTypeId.value !== '';
  const document = /^[0-9]{6,}$/.test(nroDocument.value.replace(/\D/g, ''));
...
[registerFirstName, registerLastName, registerEmail, documentTypeId, nroDocument, registerPassword].forEach(input => {
...
registerForm.addEventListener('submit', async function(e) {
  ...
  try {
    const userData = {
      ...
      document_type_id: parseInt(documentTypeId.value),
      nro_document: nroDocument.value.trim().replace(/\D/g, '')
    };
